...
console.log('— 撤回仅返还在途余量，签收/退回部分不重复返还 —')
...
assert(!cmd.dispatches.some((x) => x.id === d5.id), '撤回后记录移除')
...
// 保障量（food）：d 已办结实收 100 + d6 已签 40（挂起的在途 60 退出）
assert(cmd.sentMap[ev.id].food === 140, `挂起后保障量 = 既有实收 100 + 本单实收 40 = 140，实际 ${cmd.sentMap[ev.id].food}`)
